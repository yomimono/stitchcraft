...
let symbol_table ~font_size color_to_symbol =
  let paint_symbol description s n =
    ...
    Pdfops.[
      ...
      Op_BT;
      Op_Tj symbol;
      Op_Tf (Consts.helvetica_key, (float_of_int font_size));
      Op_Tj " : ";
      Op_Tj description;
      Op_ET;
      Op_Q;
...
let font_resources =
   Pdf.(Dictionary [
      ("/Font", Pdf.Dictionary Consts.(
        [(symbol_key, symbol_font);
         (zapf_key, zapf_font);
...
let make_page doc ~watermark ~first_x ~first_y page_number ~width ~height (pixel_fn : doc -> page -> Pdfops.t list) =
  ...
  {(Pdfpage.blankpage doc.paper_size) with
   Pdfpage.content = [
     ...
     Pdfops.stream_of_ops @@ Grid.paint_grid_lines doc page;
     Pdfops.stream_of_ops @@ Grid.label_top_grid doc page;
     Pdfops.stream_of_ops @@ Grid.label_left_grid doc page;
     Pdfops.stream_of_ops @@ add_watermark watermark;
     Pdfops.stream_of_ops @@ number_page page_number;
   ];
   Pdfpage.resources = Pdf.(Dictionary [
       ("/Font", Pdf.Dictionary Consts.([(symbol_key, symbol_font);
                                 (zapf_key, zapf_font);
                                 (helvetica_key, helvetica_font)]));
     ])
  }
