...
let init =
  ...
  in
  editor, ({ substrate; stitches = BlockMap.empty; })
...
let act_click state editor x y =
  ...
  | Block (thread, stitch) ->
    Canvas.paint_pixel state thread stitch x y
  | Eraser ->
    Canvas.erase_pixel state x y
...
let make_save_link state =
  let d = Html.window##.document in
  let image = Transformers.state_to_image state in
  let save_link = "download_png" in
  (* get the serialization from imagelib *)
  (* TODO: it should compress *real* good, allocate with that in mind *)
  let png_buf = ImagePNG.bytes_of_png image in
  match Base64.encode (Bytes.to_string png_buf) with
  | Error _  -> () (* TODO oh no *)
  | Ok png_buf ->
    let uri = Format.asprintf "data:image/png;base64,%s" png_buf in
...
let json_display state =
  ...
  let str = d##createTextNode (Js.string @@ Yojson.Safe.to_string @@ state_to_yojson state) in
...
let show_library () =
  ...
  let content =
    ...
    | 200 -> begin
      ...
      match state_of_yojson @@ Yojson.Safe.from_string patterns.Js_of_ocaml_lwt.XmlHttpRequest.content with
        | Ok state ->
