...
(* TODO: this error handling makes sense in the case where
   the base directory is present but a particular glyph might not be;
   it makes less sense in the case where the whole base is clearly
   nonsensible (it doesn't exist, it's not readable, etc) *)
let maybe_add base c m =
  match load_layer (filename base c) with
...
let map base =
  ...
  for c = 20 to 255 do
    m := maybe_add base (Uchar.of_int c) !m 
  done;
  let known_uchars = [
        0x2500 ; 0x2501
      ; 0x2660
