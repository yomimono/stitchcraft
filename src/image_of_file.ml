...
let lwt_read debug algo grid stitch ignore background input_name output =
  ...
  match Imagereader.patternfy ~algo ~grid ~stitch ~ignore ~background debug image with
...
let read debug a g s ig bg i o =
  Lwt_main.run @@ lwt_read debug a g s ig bg i o
