...
let lwt_read debug grid stitch ignore background input_name output =
  ...
  match Imagereader.patternfy ~grid ~stitch ~ignore ~background debug image with
...
let read debug g s ig bg i o =
  Lwt_main.run @@ lwt_read debug g s ig bg i o
