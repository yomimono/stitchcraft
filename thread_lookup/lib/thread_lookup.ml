...
module Commodore_64 = struct
  ...
  let thread_of_color = function
    ...
    | Blue -> dmc_lookup "796"
    (* yellow is tricky, because the displayed CSS color seems washed-out compared to the JPG sample on c64wiki.  We have a lot of bright yellows in DMC we could use. *)
    | Yellow ->
